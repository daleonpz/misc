public class RaceCondition_sync{
	...
	public static class singleton {
		...
		synchronized public void incvalue() {
			this.i++;
		}

		synchronized public void decvalue() {
			this.i--;
		}
	...
	static class rinc implements Runnable {
		...
		public void run() {
			for (int c = 0; c < 100; c++) {
				singleton.getInstance().incvalue();
			}
		}
	}
	...
	static class rdec implements Runnable {
		...
		public void run() {
			for (int c = 0; c < 100; c++) {
				singleton.getInstance().decvalue();
			}
		}
	}
